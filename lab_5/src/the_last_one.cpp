...
public:
    ...
    void execute(int set_id) override {
        ...
        std::cout << "[Набор самурая №" << set_id << "][КОНСОЛЬ]: " << message << std::endl;
    }
};
...
public:
    ...
    void execute(int set_id) override {
        ...
        if (file.is_open()) {
            file << data << "\n";
            std::cout << "[Набор самурая №" << set_id << "][Файл самурая]: Дозаписано в '" << filename << "'" << std::endl;
        } else {
            std::cerr << "[Набор самурая №" << set_id << "][Ошибка ронина]: Не удалось открыть файл " << filename << std::endl;
        }
    }
};
...
public:
    ...
    void execute(int set_id) override {
        ...
        std::lock_guard<std::mutex> lock(global_mutex);
        if (valid) {
            std::cout << "[Набор самурая №" << set_id << "][Расчёт тактики боя]: "
                      << operand1 << " " << operation << " " << operand2 << " = " << result << std::endl;
        } else {
            std::cout << "[Набор самурая №" << set_id << "][Расчёт тактики боя]: Ошибка вычисления исходов боя(" << operand1 << operation << operand2 << ")" << std::endl;
        }
    }
};
...
public:
    ...
    void execute(int set_id) override {
        for (int i = 0; i < iterations; ++i) {
            {
                std::lock_guard<std::mutex> lock(global_mutex);
                std::cout << "[Набор самурая №" << set_id << "][Путь, длинною в цикл]: Шаг на пути " << i + 1 << " из " << iterations << std::endl;
            }
...
auto now = std::chrono::system_clock::now();
    ...
    std::cout << ">>> [" << std::put_time(&tm_buf, "%H:%M:%S") << "." << std::setfill('0') << std::setw(3) << ms.count() 
              << "] Поток Набора самурая №" << set_id << " " << status << std::endl;
}
...
int main() {
    ...
    std::cout << "____________Интерпретатор 'the last of progs'____________" << std::endl;
    ...
    std::cout << "\n _______Самурай начал свой путь по выполнению наборов_______ \n" << std::endl;
    ...
    std::cout << "Все потоки завершили работу. Последняя программа завершила свой путь." << std::endl;
    return 0;
}